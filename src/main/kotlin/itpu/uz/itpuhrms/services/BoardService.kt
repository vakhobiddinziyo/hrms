...
group by b.id, u.id, ta.project_open_task_count, ta.employee_open_task_count
    ...
    override fun delete(id: Long) {
        val employee = extraService.getEmployeeFromCurrentUser()
        repository.findByIdAndDeletedFalse(id)?.let {
//            val stateExists = stateRepository.existsByBoardIdAndDeletedFalse(it.id!!)
//            if (stateExists) throw AccessDeniedException("Access denied")
    ...
    private fun saveDefaultStates(board: Board) {
        stateRepository.saveAll(
            mutableListOf(
                State(
                    DefaultTaskState.OPEN.name,
                    1,
                    board,
                    true
                ),
                State(
                    DefaultTaskState.CLOSED.name,
                    2,
                    board,
                    true
                )
