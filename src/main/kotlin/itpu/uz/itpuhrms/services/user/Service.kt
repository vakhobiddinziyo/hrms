...
import itpu.uz.itpuhrms.config.PhoneNumberConfiguration
...
class UserServiceImpl(
    ...
    override fun update(id: Long, request: UserUpdateRequest) =
        repository.findByIdAndDeletedFalse(id)?.let { user ->
            request.run {
                if (itpu.uz.itpuhrms.security.role() == Role.USER && id != userId() || user.role !in mutableListOf(Role.USER, Role.ORG_ADMIN))
                    throw AccessDeniedException("Access denied!")
                if (itpu.uz.itpuhrms.security.role() == Role.ORG_ADMIN && credentials != null)
                    throw AccessDeniedException("Access denied!")
                ...
                val userCredentials = if (itpu.uz.itpuhrms.security.role() in mutableListOf(Role.DEVELOPER, Role.ADMIN)) {
    ...
    private fun validateRequestRole(user: User, request: UserAdminUpdateRequest) {
        when {
            user.role == Role.DEVELOPER && itpu.uz.itpuhrms.security.role() != Role.DEVELOPER ->
                throw AccessDeniedException("Access denied")
