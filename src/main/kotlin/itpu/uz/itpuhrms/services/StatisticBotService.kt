...
class StatisticBotServiceImpl(
    ...
    override fun getHash(): OtpDto {
            ...
            val count = botHashIdRepository.getLastHashAmount(organization.id!!, OTP_MESSAGE_INTERVAL_LIMIT_MIN, botUsername)
            ...
            val lastMessage = botHashIdRepository.findLastHashAtInterval(organization.id!!, OTP_MESSAGE_INTERVAL_LIMIT_MIN, botUsername)
            lastMessage?.let {
                if (!it.used) {
            ...
            return OtpDto(
                hashId,
                botUsername,
                OTP_MESSAGE_RETRY_INTERVAL_LIMIT_MIN * botHashId.retryCount,
                OTP_EXPIRE_MIN
            )
    }
